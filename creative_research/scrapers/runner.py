...
Uses env for API keys. Skips a source if its env vars are missing.
"""
...
def run_all_scrapes(
    ...
    *,
    ...
    """
    Run product page fetch (or use provided text) + YouTube + Reddit + Apify (if tokens set).
    search_queries: used for YouTube search and Apify hashtags (e.g. ["skincare", "anti-aging"]).
    subreddits: e.g. ["SkincareAddiction", "30PlusSkinCare"].
    product_page_text: if provided, skip fetching product page (e.g. from pipeline).
    """
    queries = search_queries or []
    subs = subreddits or []
    data = ScrapedData(product_url=product_link)

    # Product page (use provided or fetch)
    ...
    # Reddit (public JSON API — no credentials; User-Agent: creative-research-bot/1.0)
    if queries or subs:
        try:
            ...
            data.reddit_posts_and_comments = fetch_reddit_posts_and_comments(
                subs,
                queries,
                limit_posts=max_reddit_posts,
                product_link=product_link,
            )
        except Exception:
            pass

    # Apify (TikTok, Instagram, Amazon)
    if _has_apify():
        try:
    ...
    # Tavily competitor analysis (competitors, ad library links, ad intel)
    if _has_tavily():
        try:
            from creative_research.scrapers.tavily_competitors import fetch_competitor_research
            # Use first query as category hint, or derive from product page
            category_hint = (queries[0] if queries else "") or "product"
            if data.product_page_text:
                # Use first ~500 chars of product page as category hint for Tavily
