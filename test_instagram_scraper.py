...
Verifies that run_apify_instagram returns valid reels with URLs and metadata.

Usage:
  python3 test_instagram_scraper.py [--download] [--hashtag HASHTAG]
...
def test_instagram_scrape(hashtag: str = "productreview", max_results: int = 5) -> bool:
    """Scrape Instagram reels via Apify and verify structure."""
    from creative_research.scrapers.apify_scraper import run_apify_instagram

    print(f"\n--- Instagram scrape (hashtag: #{hashtag}, max_results={max_results}) ---")
    videos = run_apify_instagram([hashtag], max_results=max_results)
...
def test_instagram_download(hashtag: str = "productreview") -> bool:
    """Download one Instagram reel. Uses direct URL when available, yt-dlp as fallback."""
    from creative_research.scrapers.apify_scraper import run_apify_instagram
    from creative_research.video_downloader import download_video

    out_dir = Path(__file__).parent / "downloads" / "test_instagram"
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"\n--- Download test (hashtag: #{hashtag}) ---")
    videos = run_apify_instagram([hashtag], max_results=5)
...
def main():
    parser = argparse.ArgumentParser(description="Test Instagram scraper via Apify")
    ...
    print("=" * 60)
    print("Instagram Scraper Test (Apify)")
    print("=" * 60)

    ok = test_instagram_scrape(hashtag=args.hashtag, max_results=args.max)
    if ok and args.download:
        ok &= test_instagram_download(hashtag=args.hashtag)
